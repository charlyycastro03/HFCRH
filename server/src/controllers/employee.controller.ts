  ...
  try {
    ...
    await db.query(
      `INSERT INTO employees (name, department, hire_date, birth_date, es_arquitecto, vacation_days_available, work_days_per_week, user_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, department, hire_date, birth_date || null, es_arquitecto ? 1 : 0, vacation_days_available || 0, work_days_per_week || 6, user_id || null]
    );
...
export async function updateEmployee(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    await db.query(
      ...
      [name, department, hire_date || null, birth_date || null, isArchitect ? 1 : 0, vacation_days_available || 0, finalWorkDays, user_id || null, id]
    );
    res.json({ success: true, msg: 'Empleado actualizado correctamente' });
  } catch (error) {
    console.error(error);
